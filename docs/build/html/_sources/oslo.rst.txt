...
Navodila za laboratorijsko vajo so zaenkrat dostopna le v *.pdf* verziji na povezavi :download:`Preračun optičnega sistema s programom OSLO LT <Preračun optičnega sistema s programom OSLO LT.pdf>`

Naloge
^^^^^^^^^^^^^^^^^^^^^^^^^^^

1. Zmodelirate optični sistem z dvema lečama.
...
.. figure:: images/oslo/2.png
	:alt: reStructuredText, the markup syntax
	:scale: 50 %
	:align: center
	
Zadnji dve služita tekstovnemu (*TW*) in grafičnemu prikazu (*GW*) rezultatov. Okno *Surface Data* je v prvi fazi modeliranja najpomembnejše, saj vanj vnašamo geometrijske
in optične podatke proučevanega lečja.

Vnos geometrijskih podatkov
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

1. V oknu *Surface Data* so torej shranjeni podatki optičnega sistema:
ime optičnega sistema:

.. figure:: images/oslo/3.png
	:alt: reStructuredText, the markup syntax
	:scale: 100 %
	:align: center
...
2. Nove površine vnesemo z desnim klikom in ukazom **Insert Before** oziroma **Insert After**. Tako vnesite štiri nove površine med *OBJ* in *AST*. Tabela mora ob koncu imeti sledeč izgled:

.. figure:: images/oslo/4.png
	:alt: reStructuredText, the markup syntax
	:scale: 100 %
	:align: center

3. Določite 3. površino kot zaslonko. To storite z desnim klikom na gumb v stolpcu **APERTURE RADIUS**:

.. figure:: images/oslo/5.png
	:alt: reStructuredText, the markup syntax
	:scale: 100 %
	:align: center

4. Vnesite razdalje med posameznimi površinami tako, da spremenite vrednosti v stolpcu **THICKENSS**. Razdaljo med objektno ravnino in 1. lečo vnesete v prvo vrstico (*OBJ*). Debelino 1. leče vnesete kot debelino 1. površine. Tako nadaljujete z vsemi debelinami, vključno z debelino 5. površine, ki popisuje razdaljo med zadnjo optično površino in slikovno ravnino.

5. Vnesite tudi krivinske radije posameznih površin. Zato spremenite vrednosti v stolpcu **RADIUS**.
.. note::
...
8. Poimenujte optični sistem, tako da v okno *Surface Data*, v polje **Lens** vpišete *LMS – vaja 2*.
...
1. V ta namen pritisnemo gumb **Draw Off**, ki se nahaja v oknu *Surface Data*. Po pritisku se gumb preimenuje v **Draw On**, izriše pa se tudi okno *Autodraw*:
...
2. Manjkajoče optične površine (zaslonko in objektno ter slikovno ravnino) izrišemo tako, da pritisnemo na gumb izbrane površine, v stolpcu **SPECIAL**, in izberemo *Surface Control*:
...
6. Isti postopek ponovimo za ostali dve površini. Na koncu mora se mora v *Autodraw* oknu izrisati sledeča slika:
...
1. V programu *OSLO LT* izrišemo žarkovno presečno krivuljo v oknu *UW 1* (za grafični prikaz) tako, da izberemo **Ray Analysis** v meniju **Setup Window** (skrajno levi gumb, zgoraj). Nato pa izberemo **RIC Plot**. V oknu se nam izriše sledeč diagram:
...
1. Podatke o lečah poiščite na spletu. Uporabite leče brez antirefleksnega nanosa, z goriščno razdaljo **50 mm** in premerom **1 inčo**. Material leče naj bo **BK7**.
